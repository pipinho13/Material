...
In order to reject the **Null Hypothesis** we want the p-value to be less than the level of significance $\alpha$ which is usually 5%. In our case we did not reject the Null Hypothesis for p=17% (p-value=`r round(t1$p.value,4)`) but we rejected for p=19% (p-value=`r round(t2$p.value,4)`)
